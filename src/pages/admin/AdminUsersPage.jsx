...
const AdminUsersPage = () => {
  ...
  const roleLabel = useCallback(
    (role) => (Array.isArray(role) ? role.join(", ") : role || "teacher"),
    []
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        key: "role",
        header: "Role",
        searchKey: [(u) => roleLabel(u?.role)],
        render: (user) => {
          const isAd = roleLabel(user.role).includes("admin");
          return (
            <div className="flex items-center gap-2">
              <div 
                className={`gap-1.5 flex items-center pr-3 pl-2.5 py-1 rounded-full text-xs font-bold border shadow-sm ${
                  isAd 
                    ? "bg-indigo-50/50 text-indigo-700 border-indigo-100/50" 
                    : "bg-slate-50 text-slate-700 border-slate-100"
                }`}
              >
                {isAd ? <Crown className="h-3.5 w-3.5" /> : <GraduationCap className="h-3.5 w-3.5 text-slate-400" />}
      ...
      },
      {
        ...
        render: (user) => {
          ...
          const subPlan = user.subscription?.planName || "Trial / Default";
          return (
            <div className="flex flex-col gap-1 min-w-[140px]">
              <div className="flex items-center">
                <Badge
                  variant={subStatus === "active" ? "success" : "default"}
                  className="uppercase text-[10px] tracking-wider font-extrabold"
                >
                  {subStatus}
                </Badge>
              </div>
              <p className="text-xs text-muted-foreground font-medium truncate capitalize">
                Plan: {subPlan}
              </p>
            </div>
          );
  ...
  const filters = useMemo(
    () => [
      ...
      },
      {
        ...
        getValue: (user) => {
          if (user?.Deleted?.isDeleted) return "revoked";
          if (user?.isDeactivated) return "suspended";
          return "active";
        },
      },
      {
        ...
        getValue: (user) => user.subscription?.status || "free",
      },
    ],
    ...
    () => [
      {
        ...
        disabled: (rows) => rows.every((u) => !u?.isDeactivated),
      },
      {
        key: "bulk-suspend",
        label: "Suspend",
      ...
      },
      {
        ...
        label: "Restore",
        variant: "outline",
        onClick: (rows) => bulkAccessAction(rows, "restore"),
        disabled: (rows) => rows.every((u) => !u?.Deleted?.isDeleted),
      },
      {
        key: "bulk-revoke",
        label: "Delete",
